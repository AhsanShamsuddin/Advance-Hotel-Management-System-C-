...
{
    public partial class RecepHomeControl : UserControl
    {
        string conString = @"Data Source=DESKTOP-EQRC7TM\ARSALAN;Initial Catalog=Hotel;User ID=sa;Password=arsalan01";
        SqlDataAdapter dataAdapter;
        DataTable table;

        public RecepHomeControl()
        {
            InitializeComponent();
        }

        private void RecepHomeControl_Load(object sender, EventArgs e)
        {
            lblTC.Text = "0";
            lblRR.Text = "0";
        }
        ...
        public void GetRoomData(string selectCommand)
        {
            try
            {
                ...
                if (table.Rows.Count > 0)
                {
                    lblTR.Text = table.Rows.Count.ToString();
                }
            }
            catch (SqlException ex)
            {
